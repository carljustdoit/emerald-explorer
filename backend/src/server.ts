...
// Trending events — reads pre-aggregated pins collection (O(1) indexed query)
app.get('/api/trending', async (req, res) => {
  try {
    if (!db) return res.json([]);
    const snap = await db.collection('pins').orderBy('count', 'desc').limit(3).get();
    const top3 = snap.docs.map(d => d.data()).filter(d => d.count > 0);
    res.json(top3);
  } catch (error) {
